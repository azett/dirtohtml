...
/**
 * This class reads properties like bit rate etc. from an MP3 file. Of course
 * these properties are read only... It supports ID3v2, i.e. it reliably skips
 * even files which tags do not use the unsynchronization scheme
 * 
 * Illegal entries are marked with special return values, not with exceptions.
 * This enables the class to continue reading other properties.
 */
public class MP3Properties {
    ...
    /**
     * Create a new instance connected to <code>file</code>. Properties are
     * read immediately.
     * 
     * @param file
     *            File to connect to
     ...
     */
    public MP3Properties(File file) throws IOException,
            NoMP3FrameException {
        readProperties(file);
    }
     ...
     *                If file does not contain at least one mp3 frame
     */
    protected void readProperties(File file) throws IOException,
            NoMP3FrameException {
        this.file = file;
        ...
        // second, third and fourth now contain the second, third and fourth
        // byte of
        ...
        samplerate = convertSamplerate(getBit(third, 3), getBit(third,
                2));
    ...
    /**
     * If ID3v2 tag present, skips it and returns length. Input stream must be
    ...
    /**
     * Sets input stream to third byte of MP3 frame header (first byte is 0xff,
     * second is consumed in synchronizing) and returns the byte already
     * consumed.
     * 
     * @param in
     *            Stream to read from
     ...
     *                If file does not contain at least one mp3 frame
     */
    protected int synchronize(FileInputStream in) throws IOException,
            NoMP3FrameException {
     ...
     * 
     * @param in1
     *            First bit to convert
     * @param in2
     *            Second bit to convert
     * @return layer
     */
    ...
    protected int convertBitrate(int in1, int in2, int in3, int in4) {
        ...
        int[][] convert = { { 0, 0, 0, 0, 0, 0 },
                { 32, 32, 32, 32, 32, 8 }, { 64, 48, 40, 64, 48, 16 },
                { 96, 56, 48, 96, 56, 24 },
                { 128, 64, 56, 128, 64, 32 },
                { 160, 80, 64, 160, 80, 64 },
                { 192, 96, 80, 192, 96, 80 },
    ...
    protected int convertMode(int in1, int in2) {
        int[] convert = { MODE_STEREO, MODE_JOINT_STEREO,
                MODE_DUAL_CHANNEL, MODE_MONO };
    ...
    protected int convertEmphasis(int in1, int in2) {
        int[] convert = { EMPHASIS_NONE, EMPHASIS_5015MS,
                EMPHASIS_ILLEGAL, EMPHASIS_CCITT };
        return convert[(in1 << 1) | in2];
    }
     ...
     * 
     * This is pretty accurate, so it *may* differ from results by many other
     * programs (like Nightmare's ID3 Tagger ;-))
     * 
     * @param id3v2_tagsize
     *            Size of ID3v2 tag
    ...
    protected long calculateLength(int id3v2_tagsize) {
         ...
         * header size is 4 bytes // 1 byte is added if padding is set, // 4
         * bytes checksum is added if protection is NOT set int headersize = 4 +
         * (padding ? 1 : 0) + (protection ? 0 : 4);
         * 
         * long filesize = file.length() - id3v2_tagsize; // #frames =
         * ceil(filesize / (framesize + headersize)) // sizeofallframes =
         * #frames * framesize // length = sizeofallframes / bitrate * 8 / 1000
         * 
         * return (long)(bitrate / (Math.ceil(filesize / (framesize +
         * headersize)) * framesize) * 0.008);
         */
    ...
    /**
     * Check if selected bit is set in <code>input</code>. This does not
     * really belong here, but it is needed...
     * 
     * @param input
     *            Value to check
     * @param bit
     *            Bit number to check (0..7 with 7 MSB)
